...
echo "sudo $FASTCLICK_DIR/bin/click --dpdk $LCORES -- fastclick/rss_2flows_v2.click &"
timeout -k 1s 17s sudo $FASTCLICK_DIR/bin/click --dpdk $LCORES -- fastclick/rss_2flow.click &

sleep 5

# run for 17 seconds, sample every 0.5s, write to cpu_load_wide.csv
python3 cpu_logger.py --duration 17 --interval 0.5 --out rss_cpu_load.csv &
